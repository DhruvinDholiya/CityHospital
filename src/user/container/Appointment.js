...
function Appointment() {
    ...
    const handleDelete = (rowData) => {
        dispatch((setAlert({ text: 'Dear, ' + rowData.name + ' Your appointment successfully deleted', color: 'success' })))
                        ...
                        tabValue === 0 ?
                            <form onSubmit={handleSubmit} className="php-email-form">
                                ...
                                <div className="text-center mt-5">
                                    <Button btnStyle='submit'>{apt.loading ? 'Please wait....' : 'Make an Appointment'}</Button>
                                </div>
                            </form>
                            : apt.apt.length > 0 ?
